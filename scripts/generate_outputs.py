...
def generate_readme(papers, recent, now):
    ...
    lines.append(f'> 📊 **{len(papers)} papers** | ⭐ Dual-Channel: {len(both)} | ⏰ {now.strftime("%Y-%m-%d %H:%M")} UTC+8')
    lines.append('')
    lines.append('📥 [Excel](COCHE_Papers.xlsx) | [JSON](coche_pubmed.json) | [Full Table](FULL_LIST.md) | [Report](COCHE_Weekly_Report.md)')
    lines.append('')
    lines.append('## ⭐ What Does the Star Mean?')
    lines.append('')
    lines.append('**⭐ Dual-Channel** = A paper matches **BOTH** of the following criteria required by ITC/InnoHK KPI reporting:')
    lines.append('')
    lines.append('1. **COCHE Affiliation** — at least one author lists COCHE as their institutional affiliation')
    lines.append('2. **InnoHK/ITC Acknowledgement** — the paper explicitly acknowledges InnoHK, the Innovation & Technology Commission (ITC), or the Hong Kong Centre for Cerebro-Cardiovascular Health Engineering (COCHE) in its funding/acknowledgement section')
    lines.append('')
    lines.append('Papers matching only one channel (affiliation-only or acknowledgement-only) are collapsed by default and shown without the ⭐ badge. Only dual-channel papers count toward official ITC KPIs.')
    lines.append('')
    lines.append('---')
    lines.append('')
    ...
    if both_recent:
        lines.append(f'## ⭐ Recent — Dual-Channel ({len(both_recent)})')
        lines.append('')
    ...
    if other_recent:
        lines.append(f'<details><summary>📋 Other Recent ({len(other_recent)} papers, single-channel)</summary>')
        lines.append('')
        for p in other_recent:
            tag = ' 🏷 InnoHK-only' if p.get('source')==['innohk_acknowledgement'] else ''
    ...
    lines.append('## 📋 All Papers by Year')
    lines.append('')
    for year in sorted(year_counts.keys(), reverse=True):
        ...
        if yr_single:
            ...
            tail = ''
            if yr_single_affil: tail += f' +{len(yr_single_affil)} affiliation'
            if yr_single_innohk: tail += f' +{len(yr_single_innohk)} InnoHK'
            lines.append(f'<details><summary>More papers{tail} (single-channel)</summary>')
            for p in yr_single:
                tag = ' 🏷 InnoHK-only' if p.get('source')==['innohk_acknowledgement'] else ''
                lines.append(f'- [{p["title"][:80]}]({format_link(p)}) ({format_date(p)}){tag}')
            lines.append('</details>')
        ...
        lines.append('')
    lines.append('---')
    lines.append('> **⭐ Star = Dual-Channel**: paper matches BOTH COCHE affiliation AND InnoHK/ITC acknowledgement (ITC KPI compliant).  ')
    lines.append('> Single-channel papers (affiliation-only or InnoHK-only) are collapsed by default — click to expand.')
    lines.append('')
...
def generate_full_list(papers, now):
    ...
    lines.append('')
    lines.append(f'> 📊 **{len(papers)}** papers | ⭐ Dual-Channel: {len(both)} | ⏰ {now.strftime("%Y-%m-%d %H:%M")} UTC+8 | [Home](README.md)')
    lines.append('')
    lines.append('> **⭐ Dual-Channel** = COCHE affiliation + InnoHK/ITC acknowledgement (counts toward ITC KPI). Single-channel papers use `-` numbering. See [README](README.md) for full explanation.')
    lines.append('')
    lines.append('---')
    lines.append('')
    year_counts = Counter(p['pub_year'] for p in papers)
    for year in sorted(year_counts.keys(), reverse=True):
        ...
        for i, p in enumerate(yr_both, 1):
            ...
            lines.append(f'| {i} | ⭐ [{title_escaped[:100]}]({format_link(p)}) | {format_date(p)} | {(p.get("journal","") or "N/A")[:30]} | {format_authors(p, 99, "")} | **Dual-Channel** | {p["pmid"]} |')
        for p in yr_single:
            is_innohk = p.get('source')==['innohk_acknowledgement']
            title_escaped = p['title'].replace('|', '\\|')
            tag = '🏷 InnoHK-only' if is_innohk else 'affiliation'
            lines.append(f'| - | {title_escaped[:100]}]({format_link(p)}) | {format_date(p)} | {(p.get("journal","") or "N/A")[:25]} | {format_authors(p, 99, "")} | {tag} | {p["pmid"]} |')
        lines.append('')
    lines.append('---')
    lines.append('> **⭐ Dual-Channel** = COCHE affiliation + InnoHK/ITC acknowledgement (ITC KPI compliant) | Single-channel papers use `-` numbering')
...
def generate_report(papers, recent, now):
    ...
    lines.append(f'- Total: {len(papers)} | ⭐ Dual-Channel: {len(both)} | InnoHK-only: {innohk_only}')
    lines.append(f'- Past 30d: {len(recent)} (Dual-Channel: {len(both_recent)})')
    lines.append('')
    lines.append('> **⭐ Dual-Channel** = COCHE institutional affiliation + InnoHK/ITC acknowledgement. These count toward official ITC KPIs.')
    lines.append('')
    if both_recent:
        lines.append('## ⭐ Recent — Dual-Channel (Affiliation + InnoHK/ITC)')
        lines.append('')
    ...
    other_recent = [p for p in recent if p not in both_recent]
    if other_recent:
        lines.append('## 📋 Other Recent (Single-Channel)')
        lines.append('')
        for p in other_recent:
            tag = '🏷 InnoHK-only' if p.get('source')==['innohk_acknowledgement'] else 'affiliation'
            lines.append(f'- [{p["title"][:60]}]({format_link(p)}) | {format_date(p)} | {tag}')
        lines.append('')
    lines.append('---')
    lines.append(f'*Generated {now.strftime("%Y-%m-%d %H:%M:%S")} · ⭐ = Dual-Channel (affiliation + InnoHK/ITC)*')
    lines.append('')
    return '\n'.join(lines)
...
def generate_index(papers, recent, now):
    ...
    lines.append('')
    lines.append('> **Hong Kong Centre for Cerebro-Cardiovascular Health Engineering**')
    lines.append('> Weekly auto-update · ⭐ Star = Dual-Channel (COCHE affiliation + InnoHK/ITC acknowledgement)')
    lines.append('')
    lines.append(f'📊 **Total: {len(papers)}** | ⭐ Dual-Channel: {len(both)} | 🆕 Past 30d: {len(recent)} | ⏰ {now.strftime("%Y-%m-%d %H:%M")}')
    lines.append('')
    lines.append('---')
    lines.append('')
    if both_recent:
    ...
    for year in sorted(year_counts.keys(), reverse=True):
        ...
        for i, p in enumerate(yr_both, 1):
            title = p['title'][:70] + ('...' if len(p['title']) > 70 else '')
            lines.append(f'| {i} | ⭐ [{title}]({format_link(p)}) | {format_date(p)} | {(p.get("journal","") or "N/A")[:26]} | Dual-Channel |')
        if yr_single:
            yr_si = [p for p in yr_single if p.get('source')==['innohk_acknowledgement']]
            yr_sa = [p for p in yr_single if p not in yr_si]
            tail = ''
            if yr_sa: tail += f' +{len(yr_sa)} affiliation'
            if yr_si: tail += f' +{len(yr_si)} InnoHK'
            lines.append(f'<details><summary>Single-channel papers{tail}</summary>')
            for p in yr_single:
                tag = '🏷 InnoHK-only' if p.get('source')==['innohk_acknowledgement'] else 'affiliation'
                title = p['title'][:70] + ('...' if len(p['title']) > 70 else '')
    ...
    lines.append('')
    lines.append(f'*{now.strftime("%Y-%m-%d %H:%M:%S")} · PubMed API · ⭐ = Dual-Channel (COCHE affiliation + InnoHK/ITC acknowledgement)*')
    lines.append('')
    return '\n'.join(lines)
...
def generate_excel(papers, threshold_str):
    ...
    for idx, p in enumerate(display_order, 1):
        ...
        if is_both:
            status_label = '⭐ Dual-Channel'
        elif is_innohk_only:
            status_label = '🏷 InnoHK-only'
        else:
            status_label = 'affiliation'
    ...
    ws.cell(row=legend_row, column=2, value=f'⭐ Dual-Channel = COCHE affiliation + InnoHK/ITC acknowledgement ({both_count} papers) | Gold highlight = Dual-Channel + published past 30 days ({recent_both} papers) | Grey text = single-channel')
